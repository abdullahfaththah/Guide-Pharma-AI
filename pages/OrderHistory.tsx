...
import { FileText, History as HistoryIcon, CheckCircle, Eye, X, AlertTriangle } from 'lucide-react';
...
export const OrderHistory: React.FC = () => {
  ...
  const generatePDF = () => {
    ...
    doc.text("GUIDE-PHARMA - Order History", 14, 22);
    
    doc.setFontSize(11);
    doc.setTextColor(100);
    doc.text(`Generated on: ${new Date().toLocaleDateString()}`, 14, 30);

    const tableData = historyOrders.map(order => [
      order.id,
      order.date,
      order.items.length,
    ...
    autoTable(doc, {
      ...
      startY: 40,
      theme: 'grid',
      headStyles: { fillColor: [75, 85, 99] } // Gray for history
    });
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">View your past transactions and receipts.</p>
        </div>
